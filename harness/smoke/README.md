...
| [`linux_sandbox.py`](linux_sandbox.py) | The same four boundaries under bubblewrap. It asserts the backend really is bubblewrap — a run that fell back and still passed would be reporting on a boundary it never tested. **Manual only:** an earlier hosted-runner job failed during private-network setup; the packaged AppArmor profile used by the new interrupt job may change that result, but this full filesystem-and-egress smoke has not yet been re-evaluated there. See [`docs/platforms.md`](../../docs/platforms.md) for what the Linux tier now claims. |
