...
- CLI: `MCScanX -h`; `syri --version`; `python3 -m jcvi.compara.catalog ortholog --help`
...
- CLI: `python3 -m jcvi.compara.catalog ortholog A B` for JCVI/MCScan Python pipeline
...
python3 -m jcvi.formats.gff bed --type=gene --key=ID species_A.gff > A.gff.tmp
python3 -m jcvi.formats.gff bed --type=gene --key=ID species_B.gff > B.gff.tmp
...
python3 -m jcvi.formats.gff bed --type=mRNA --key=Name species_A.gff > A.bed
python3 -m jcvi.formats.gff bed --type=mRNA --key=Name species_B.gff > B.bed
python3 -m jcvi.formats.fasta format species_A.faa A.fasta
python3 -m jcvi.formats.fasta format species_B.faa B.fasta

python3 -m jcvi.compara.catalog ortholog --no_strip_names A B
# Produces A.B.lifted.anchors and other files

# 2. Dotplot
python3 -m jcvi.graphics.dotplot A.B.anchors --dpi 300 --output dotplot.png
...
EOF
python3 -m jcvi.graphics.karyotype seqids.txt layout.csv

# 4. Microsynteny block plot
python3 -m jcvi.graphics.synteny blocks.layout
```
