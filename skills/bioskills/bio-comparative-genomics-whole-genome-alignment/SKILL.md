...
**Fix:** Set `--ploidy` to query genome's ploidy (1 for haploid reference, 2 for diploid query when reference is diploid, 4 for tetraploid query vs diploid reference). Check with `python3 -m jcvi.compara.synteny depth` on output anchors.
