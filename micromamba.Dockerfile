...
# install micromamba, avoid architectural problems
...
ARG OPENAI4S_INSTALL_R_ENV=false
# Set to true when building an offline-capable runtime image, install all environments
ARG OPENAI4S_OFFLINE_RUNTIME=false
...
RUN mkdir -p /opt/mamba/envs
# set micromamba env vars
...
RUN set -eu; \
    ...
    python -m pip install --index-url "${PIP_INDEX_URL}" --trusted-host "${PIP_TRUSTED_HOST}" --no-cache-dir "${wheel}"; \
    rm -rf /tmp/wheels

# install environments
RUN if [ "${OPENAI4S_OFFLINE_RUNTIME}" = "true" ] || [ "${OPENAI4S_INSTALL_R_ENV}" = "true" ]; then \
        micromamba env create -f /usr/local/lib/python3.12/site-packages/envs/r.yml -y \
        && micromamba clean -afy; \
    fi
RUN if [ "${OPENAI4S_OFFLINE_RUNTIME}" = "true" ]; then \
        for yml in /usr/local/lib/python3.12/site-packages/envs/*.yml; do \
            [ -f "$yml" ] || continue; \
            [ "$(basename "$yml" .yml)" = "r" ] && continue; \
            micromamba env create -f "$yml" -y; \
        done \
        && micromamba clean -afy; \
    fi
RUN if [ -d /opt/mamba ]; then chown -R 1000:1000 /opt/mamba; fi
